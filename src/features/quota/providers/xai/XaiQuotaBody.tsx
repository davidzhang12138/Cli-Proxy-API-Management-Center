    ...
    return (
      <>
        {quota.resources.map((resource, index) => {
          const remaining =
            resource.remaining ??
            (resource.totalLimit !== null && resource.currentUsage !== null
              ? Math.max(0, resource.totalLimit - resource.currentUsage)
              : null);
          const remainingPercent =
            resource.totalLimit !== null && resource.totalLimit > 0 && remaining !== null
              ? Math.max(0, Math.min(100, (remaining / resource.totalLimit) * 100))
              : resource.exhausted
                ? 0
                : null;
          const normalizedType = resource.resourceType.trim().toLowerCase();
          const resourceKey = `xai_quota.resource_${normalizedType}`;
          const translated = t(resourceKey);
          const label =
            translated === resourceKey ? t('xai_quota.resource_quota') : translated;
          const resetLabel = formatQuotaResetTime(resource.resetAt);

          return (
            <div key={`${normalizedType}:${index}`} className={classes.quotaRow}>
              <div className={classes.quotaRowHeader}>
                ...
                <div className={classes.quotaMeta}>
                  ...
                  <span className={classes.quotaAmount}>
                    {formatXaiRateLimitAmount(remaining, resource.totalLimit)}
                  </span>
                  {resetLabel !== '-' && (
