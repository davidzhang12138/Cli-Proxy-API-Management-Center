...
const buildXaiQuotaStateFromUsageQuota = (file: AuthFileItem): XaiQuotaState | null => {
      ...
      const order = (resourceType: string) =>
        resourceType.toLowerCase() === 'requests'
          ...
          : resourceType.toLowerCase() === 'tokens'
            ? 1
            : 2;
      return order(left.resourceType) - order(right.resourceType);
    });

  return resources.length > 0 ? { status: 'success', billing: null, resources } : null;
};
...
export const XAI_CONFIG: QuotaProviderData<XaiQuotaState, XaiBillingSummary> = {
  ...
  buildSnapshotState: buildXaiQuotaStateFromUsageQuota,
};
