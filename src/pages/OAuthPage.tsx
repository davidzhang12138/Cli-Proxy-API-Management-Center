...
import { oauthApi, freebuffAuthApi, type FreebuffStatusRequest } from '@/services/api/oauth';
...
export function OAuthPage() {
  ...
  const startPolling = (
    ...
    freebuff?: FreebuffStatusRequest
  ) => {
    attempt.poll(
      async () => {
        if (provider === 'freebuff' && freebuff) {
          const res = await freebuffAuthApi.getStatus(freebuff, attempt.signal);
          return {
            status:
              res.status === 'pending'
                ? 'wait'
                : res.status === 'ok' && res.token_added
                  ? 'ok'
                  : 'error',
            error: res.error,
          };
        }
        return oauthApi.getAuthStatus(state, attempt.signal);
      },
      (res) => {
        if (res.status === 'ok') {
  ...
  const startAuth = async (provider: string) => {
    ...
    try {
      if (provider === 'freebuff') {
        ...
        startPolling(provider, fingerprintId, attempt, {
          fingerprintId,
          fingerprintHash,
          expiresAt,
          ...(proxyUrl ? { proxyUrl } : {}),
