...
const acceptsUsageApi = (api: typeof usageApi) => {
  const canFetchUsageWithDateParams = api.getUsage({
    start: '2026-06-01',
    end: '2026-06-02',
  });
  void canFetchUsageWithDateParams;

  const canFetchAuthUsage = api
    .getAuthUsage({
      auth_index: 'auth-1',
  ...
  const canFetchModelPrices = api.getModelPrices().then((response) => {
  ...
  const canReplaceModelPrices = api.replaceModelPrices({
    'gpt-5.5': {
      prompt: 1.25,
      completion: 2.5,
      cache: 0.125,
