...
describe('Muse quota UI integration', () => {
  ...
  test('invalidates file-scoped and session caches and rejects stale writes', () => {
    ...
    // stampQuotaMap re-wraps every committed entry with its own cache
    // timestamps, so compare the payload the entry carries.
    expect(useQuotaStore.getState().metaQuota['other.json']).toMatchObject(quota);
